...
const PageTransition: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTransition === 'LOGIN' || activeTransition === 'LOGOUT') {
      const isLogin = activeTransition === 'LOGIN';
      ...
      if (isLogin) {
        ...
        gsap.set(overlayRef.current, { x: '100%' });
        gsap.set(pathRef.current, { strokeDashoffset: 0, fill: "#10B77F" });
      }
          ...
          setTimeout(() => {
            gsap.to(overlayRef.current, {
              ...
              onComplete: () => {
                ...
                // Reset to default visually hidden
                gsap.set(overlayRef.current, { x: '-100%' });
              }
            });
          }, 300);
      ...
      if (isLogin) {
        // 2. Draw outline
        tl.to(pathRef.current, { strokeDashoffset: 0, duration: 0.5, ease: 'power2.inOut' })
          // 3. Fill logo
          .to(pathRef.current, { fill: '#10B77F', duration: 0.2, ease: 'power2.out' }, "-=0.1");
      } else {
        // Logout: Unfill logo
        tl.to(pathRef.current, { fill: "transparent", duration: 0.2, ease: 'power2.out' })
          // Undraw outline
          .to(pathRef.current, { strokeDashoffset: 1000, duration: 0.5, ease: 'power2.inOut' }, "-=0.1");
      }
    }
  }, [activeTransition, dispatch]);

  // Render nothing if not a global transition (so we don't trap pointer events if something breaks)
  if (activeTransition !== 'LOGIN' && activeTransition !== 'LOGOUT') return null;

  return (
    <div 
      ref={overlayRef}
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-white dark:bg-[#0d1f2d] will-change-transform"
      style={{ transform: activeTransition === 'LOGIN' ? 'translateX(-100%)' : 'translateX(100%)' }}
    >
      <svg 
        ref={logoRef}
        width="100" 
